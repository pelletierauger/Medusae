...
var windsOfSpring015 = new System({
    translate: {
        x: 0,
        y: 0
    },
...
var windsOfSpring016 = new System({
    translate: {
        x: 0,
        y: 0
    },
    ...
    trailFunction: function(x, y) {
        var newX = sin(x / 50);
        var newY = sin(y / 15) * cos(y / 5);
        return { x: newX * 200, y: newY * 60 };
    },
    displayFunction: function(h) {
        var size = 1;
        for (var i = 0; i < (h.length - 1); i++) {
...
var windsOfSpring017 = new System({
    translate: {
        x: 0,
        y: 0
    },
    ...
    updateFunction: function(t) {
        return {
            x: cos(t / 5) * sin(t / 7.5) * 15,
            y: sin(t / 5) * 15
        }
    }
});

var windsOfSpring018 = new System({
    translate: {
        x: 565,
        y: 0
    },
    ...
    updateFunction: function(t) {
        return {
            x: cos(t / 5) * cos(t / 5) * 15,
            y: cos(t / 15) * 15
        }
    }
});

var windsOfSpring019 = new System({
    translate: {
        x: 0,
        y: 600
    },
    ...
    rotate: Math.PI / 2,
    trailFunction: function(x, y) {
        var newX = sin(x / 10) * cos(x / 5);
        var newY = sin(y / 5) * sin(y / 25);
        return { x: newX * 55, y: newY * 130 };
    },
    displayFunction: function(h) {
        var size = 1;
        for (var i = 0; i < (h.length - 1); i++) {
    ...
    updateFunction: function(t) {
        return {
            x: Math.pow(sin(t / 15), 3) * 15,
            y: cos(t / 15) * 15
        }
    }
});

var windsOfSpring020 = new System({
    translate: {
        x: 0,
        y: 0
    },
    ...
    displayFunction: function(h) {
        var sizeRandom = 10 + this.sumOfIterations / 200;
        var size = 1;
        for (var i = 0; i < (h.length - 1); i++) {
            var pos = h[(h.length - 1) - i];
            var maps = sketch.map(Math.abs(pos.y), 0, sketch.height / 2, 1, 0);
            var alpha = sketch.map(i, 0, h.length, 0, 255);
            sketch.push();
            sketch.translate(pos.x, pos.y);

            var col4 = sketch.color(90, 0, 255, alpha);
            var col5 = sketch.color(255, 150, 30, alpha);
            var col6 = sketch.lerpColor(col4, col5, maps);
            sketch.fill(col6);
            // sketch.fill(255, alpha);
            sketch.ellipse(0, 0, size, size);

            var col1 = sketch.color(0, 0, 255, alpha * 0.1);
            var col2 = sketch.color(255, 0, 0, alpha * 0.1);
            var col3 = sketch.lerpColor(col1, col2, maps);
            for (var j = 0; j < 30; j++) {
                var sX = sketch.random(-sizeRandom, sizeRandom);
                var sY = sketch.random(-sizeRandom, sizeRandom);
                var mappy = sketch.map(Math.abs(sX) + Math.abs(sY), 0, 20, 255, 0);
...
var windsOfSpring021 = new System({
    translate: {
        x: 420,
        y: 0
    },
    ...
    trailFunction: function(x, y) {
        var newX = sin(x / 50);
        var newY = sin(y / 25);
        return { x: newX * 120, y: newY * 42 };
    },
    displayFunction: function(h) {
        var size = 1;
        var sizeRandom = 10 + this.sumOfIterations / 100;
        for (var i = 0; i < (h.length - 1); i++) {
            var pos = h[(h.length - 1) - i];
            var alpha = sketch.map(i, 0, h.length, 0, 255);
            sketch.push();
            ...
            alpha = 155;
            var p = 15;
            var wallow = sketch.map(Math.sin(i / 12), -1, 1, 20, 255);
            var wallow2 = sketch.map(Math.cos(i / 6), -1, 1, 50, 255);
            // var mapsWidth = sketch.map(pos.x - 420, 0, sketch.width / 2, 1, 0.1);
            for (var k = 0; k < p; k++) {
                var mapAlpha = sketch.map(k, 0, p, 0.001, 0.05);
                var mapSize = sketch.map(k, 0, p, 10, 0.05);
                // sketch.fill(255, wallow, wallow2, alpha * mapAlpha / 2);
                sketch.fill(255, alpha * mapAlpha / 2);
                sketch.ellipse(0, 0, size * mapSize);
            }
            ...
            for (var j = 0; j < 30; j++) {
                ...
                var sY = sketch.random(-sizeRandom, sizeRandom);
                var mappy = sketch.map(Math.abs(sX) + Math.abs(sY), 0, 20, 255, 0);
                // sketch.fill(255, alpha * 0.01);
                // sketch.fill(255, wallow, wallow2, alpha * 0.125);
                sketch.fill(255, alpha * 0.125 * 0.6);
                sketch.ellipse(sX, sY, size / 2, size / 2);
            }
            sketch.pop();
        }
...
var windsOfSpring022 = new System({
    translate: {
        x: 550,
        y: 0
    },
        ...
        for (var i = 0; i < (h.length - 1); i++) {
            if (i > 15 && i < 80) {
                ...
                for (var k = 0; k < p; k++) {
                    ...
                    var mapSize = sketch.map(k, 0, p, 10, 0.05);
                    // sketch.fill(255, wallow, wallow2, alpha * mapAlpha / 2);
                    sketch.fill(0, alpha * mapAlpha / 2);
                    sketch.ellipse(0, 0, size * mapSize);
                }
...
var windsOfSpring023 = new System({
    translate: {
        x: 550,
        y: 0
    },
        ...
        for (var i = 0; i < (h.length - 1); i++) {
            if (i > 15 && i < 80) {
                ...
                for (var j = 0; j < 30; j++) {
                    ...
                    // sketch.fill(255, alpha * 0.01);
                    // sketch.fill(255, wallow, wallow2, alpha * 0.125);
                    sketch.fill(255, alpha * 0.125 * 0.3);
                    sketch.ellipse(sX, sY, size / 2, size / 2);
                }
                sketch.pop();
            }
